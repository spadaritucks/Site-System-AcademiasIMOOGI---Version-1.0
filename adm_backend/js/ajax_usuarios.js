...
function verificarModalidades() {
    ...
    $.ajax({
        ...
        let planosResult = planos.PlanosAtivos;
...
function listarPlanoFicha() {
    $.ajax({
        ...
        let PlanosAtivos = result.PlanosAtivos
        for (let i = 0; i < PlanosAtivos.length; i++) {
            $('#planosArea').append(`
            <option value="${PlanosAtivos[i].id_planos}">${PlanosAtivos[i].nome_plano}</option>`)
